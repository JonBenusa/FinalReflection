...
void fillBoardFromFile (int board[30][30])
{
    ...
    fp = fopen(filename, "r");//    C:\testFiles\Lab10NoCreaturesTest.txt
    if (fp==NULL)
    {
...
int checkLife(int board[30][30], int row, int col)
{
    ...
    while(x<=1+row)//as long as x is a col neighboring or containing the current check
    {
        ...
        {
            if((x!=row || y!=col) && (x>=0 && y>=0) && (x<=29 && y<=29))//check to make sure that the neighbor is actually on the board and that it isn't the one currently being checked
            {
...
void randFillBoard(int board[30][30])
{
    ...
    int col = 29;
    int alive;
    while(row>=0)//for each spot in the matrix
    {
        col = 29;
        while(col>=0)
        {
            alive = rand()%10;//ten percent chance of getting an animal
            if(alive==0)
            {
                board[row][col] = 1;//if it's alive put in a 1
            }
            else
            {
                board[row][col] = 0;//if it's dead put in a 0
            }
            col = col - 1;
        }
        row = row - 1;
...
int reaction(int board[30][30],int whatHappens[30][30], int stillAlive)
{
    int row = 0;
    int col = 0;
    stillAlive = 0;
...
void main()
{
    ...
    while(nextGen=='y' && stillAlive==1)
    {
        stillAlive = reaction(board, whatHappens, stillAlive);//creates a  new board based on the previous round results
