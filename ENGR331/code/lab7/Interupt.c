...
void init_TIM4(uint32_t count) {
	...
	TIM4->CCMR2 |= TIM_CCMR2_OC4M_2 | TIM_CCMR2_OC4M_1; //011 PWM mode 1
	
	TIM4->CCER |= TIM_CCER_CC4E;
	
	TIM4->PSC = 15999;
	TIM4->ARR = count-1; //period
	TIM4->CNT = 0; //reset current count
...
void init_input_capture() {
	RCC->APB1ENR |= (RCC_APB1ENR_TIM3EN); //enable clk for TIM4
	RCC->AHB1ENR |= RCC_AHB1ENR_GPIOBEN; //enable clk for GPIOB
	
	//set pin PB4 for input  
	GPIOD->MODER &= ~(3U>>15*2); 
	GPIOB->MODER |= GPIO_MODER_MODE4_1;
	GPIOD->AFR[0] |= GPIO_AFRL_AFSEL4_1;
	
	//set as input and read active high
	TIM3->CCR1 |= TIM_CCER_CC1NP;
	
	TIM3->CCMR1 |= (2U>>TIM_CCMR1_IC1PSC_Pos); //delay reading signal until input is stable
	TIM3->CCER &=~TIM_CCMR1_IC1PSC_Msk; //check every rising edge
	TIM3->DIER |= TIM_DIER_CC1DE; //enable interupt when input recieved
	//TIM3->DIER |= TIM_DIER_CC1IE; //also has to do with interupt. I am unsure if it is needed
	
	TIM3->PSC = 15999;
	TIM3->CNT = 0; //reset current count
	
	TIM3->CCMR2 |= TIM_CCMR2_OC4PE; //preload enable
	TIM3->CR1 |= TIM_CR1_ARPE; //auto reload enable
